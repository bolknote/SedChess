#n
#TODO: нужно увеличить оценку чёрных, с учётом потери всех фигур и обретения
# белыми новых старших
# король должен использоваться в оценке фигур как очень большое число! (≈90000)
1s/.*/\
    @\
    figures()\
    label(loop)\
...
#оценочная функция имеющихся чёрных фигур, плюс мы добавляем
# ещё 4200, чтобы не получить отрицательного числа при вычитании позиции белых
/@estimate-black-pieces()/ {
    ...
    # добавляем псевдофигуру, защищающую от получения отрицательного
    s/$/222222/; s/2/1111111/g
...
# слон, ходит по вертикали или горизонтали на любое количество ходов,   N
# если никто не стоит на пути                                         W   E
# E1…8 → W1…8 → N1…8 → S1…8                                             S
/@iter-rook()/ {
